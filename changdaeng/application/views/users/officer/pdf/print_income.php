...
// กำหนดเนื้อหาข้อมูลที่จะสร้าง pdf ในที่นี้เราจะกำหนดเป็นแบบ html โปรดระวัง EOD; โค้ดสุดท้ายต้องชิดซ้ายไม่เว้นวรรค
    $pdf->Ln(1);
    $heading = <<<EOD
            <h3 style="color:#0a3d62"><b>ข้อมูลแสดงรายละเอียดรายรับ</b></h3>
EOD;
    ...
    $top_table = '
            <table cellspacing="0" cellpadding="1">
                <tr>
                    <td colspan="3"></td>
                    <td></td>
                    <td colspan="2" align="right">'.datethai(date("d M Y", strtotime($time_print))).'&nbsp;'.date('H:i:s').'</td>
                </tr>
            </table>';
    ...
    $data_table .= '<tr>
                        <th align="center" width="8%">ลำดับ</th>
                        <th align="center" width="25%">รหัสการนวด</th>
                        <th align="center">จำนวนเงิน</th>
                        <th align="center" width="42%">วันที่</th>
                    </tr>';
    $total_income = 0;
    $count = 1;
    foreach ($income as $value) {
        $total_income += $value->money;
        $data_table .= '<tr>
                            <td align="center">'.$count++.'</td>
                            <td align="left">'.$value->detail_list_id.'</td>
                            <td align="right">'.number_format($value->money).'</td>
                            <td align="center">'.dateTimeThai(date("d M Y H:i:s", strtotime($value->date))).'</td>
                        </tr>';
    }
    $data_table .= '</table>';
    if(!empty($income)){
