                ...
                <div class="row page-titles">
                    <div class="col-md-5 align-self-center">
                        <h3 class="text-themecolor">จัดการข้อมูลสมาชิก</h3>
                    </div>
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            ...
                            <div class="card-body">
                            <button type="button" class="btn btn-info btn-rounded m-t-10 float-right" data-toggle="modal" data-target="#add-room">เพิ่มสมาชิก</button>
                                    ...
                                    <thead class="bg-dark">
                                        ...
                                        <?php foreach ($query as $key => $value) { ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="javascript:void(0)" class="badge badge-warning editA" data-toggle="modal" data-target="#edit" id="<?= $value->member_id;?>"> 
                                                        <i class="fas fa-pencil-alt"> แก้ไข</i>
                                                    </a>
                                                    <a href="javascript:void(0)" onclick="deleteMember(<?=$value->member_id;?>)" class="badge badge-danger">
    ...
    function deleteMember(id){
        swal({
            ...
            if (result.value) {
                $.ajax({
                    type: "post",
                    url: "<?= base_url('admin/deleteMember');?>",
                    data: {"id":id},
                    dataType: 'json',
                    success: (data)=>{
                        if(data.status===true){
...
$(document).ready(function() {
    $('#addU').validate({
                rules:{
                    email:{
                        ...
                        remote:{
                            type: "POST",
                            url: "<?=base_url('admin/chk_mail_member');?>",
                            data:{
                                email: function(){
                                    return $("#email").val();
                                }
                    ...
                    tel:{
                        ...
                        remote:{
                            type: "POST",
                            url: "<?=base_url('admin/chk_tel_member');?>",
                            data:{
                                tel: function(){
                                    return $("#tel").val();
                                }
    ...
    $('.editA').click(function(){
        ...
        $.ajax({
            type: "post",
            url: "<?= base_url('admin/getMemberById');?>",
            data: {id:id},
            dataType: 'json',
            success:function(data){
                console.log(data);
                $('#memberId').val(data["userById"][0].member_id);
                ...
                $('.img').attr("src","<?= base_url('upload/member/');?>"+data["userById"][0].img);
            }
        });
        ...
        submitHandler: (form,e)=>{
            ...
            swal({
                ...
                if(result.value){
                    $.ajax({
                        type: "post",
                        url: "<?= base_url('admin/edit_member');?>", 
                        data: data,
                        dataType: 'json',
                        async: false,
                        cache: false,
